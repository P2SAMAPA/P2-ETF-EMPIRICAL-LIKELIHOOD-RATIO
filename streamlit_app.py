...
st.set_page_config(page_title="Empirical Likelihood Ratio", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Empirical Likelihood Ratio Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Macro‑conditional test for positive mean return</p>', unsafe_allow_html=True)
...
st.sidebar.markdown(f"**Primary macro:** {config.PRIMARY_MACRO} | **Min obs:** {config.MIN_OBS}")
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Likelihood ratio: {etf['el_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized LR"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized LR", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Empirical Likelihood Ratio (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Empirical likelihood** (Owen 2001) provides non‑parametric likelihood inference without distributional assumptions.
        - For each ETF, we test H₀: mean return ≤ 0 vs H₁: mean > 0, conditional on the current macro regime (e.g., VIX high/low).
        - The score is the empirical likelihood ratio statistic (2 log sup L / L₀). Larger values indicate stronger evidence for positive mean.
        - Uses only the subsample of returns that fall in the same macro regime as today.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the likelihood ratio scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Empirical Likelihood Ratio | Macro‑conditional test for positive mean")
