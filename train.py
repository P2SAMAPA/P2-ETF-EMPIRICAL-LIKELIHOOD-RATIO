...
from empirical_likelihood import el_ratio_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    if macro_df is not None:
        ...
        macro_window = None
    if macro_window is None or config.PRIMARY_MACRO not in macro_window.columns:
        return None
    # Get the macro series (within window) and today's macro value
    macro_series = macro_window[config.PRIMARY_MACRO].values
    if len(macro_series) == 0:
        return None
    macro_today = macro_series[-1]
    # Compute score for each ETF
    raw_scores = {}
    ...
    top_etfs = [{"ticker": t, "el_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "min_observations": config.MIN_OBS,
        "universes": {}
    }
    ...
    out_file = f"output/empirical_likelihood_{timestamp}.json"
